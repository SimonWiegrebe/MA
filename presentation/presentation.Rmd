...
- \textit{CT} has the best $HR@k, \forall k \in \{1,5\}$, and the best $MRR@k, \forall k \in \{1,5,10,20\}$.
- \textit{VSTAN\_EBR} performs best in terms of $HR@20$, i.e., when the Hit Rate is evaluated on the list of the top 20 recommendations per event.
- The three NN-based models \textit{GRU4Rec}, \textit{GRU4Rec\_R}, and \textit{HGRU4Rec} come right behind \textit{CT} i.t.o. $HR@1$, yet their relative performance deteriorates as $k$ increases.
...
- Hypotheses:
  - We expect \textit{GRU4Rec}, \textit{Gru4Rec\_R}, and \textit{HGRU4Rec} to perform better on longer sequences, where the underlying RNNs are provided more events to learn from.
  - We expect sequence length to have no impact on performance of \textit{AR}, \textit{SR}, and \textit{SR\_BR}, since they only use the last event of a sequence for prediction.
...
- \textit{CT} still has the best $HR@k, \forall k \in \{1,5\}$, and the best $MRR@k, \forall k \in \{1,5,10,20\}$.
- No large changes for \textit{AR}, \textit{SR}, and \textit{SR\_BR}.
- Performance of NN-based models improves $\rightarrow$ RNN-based models benefit from being trained and evaluated on longer sequences.
- What if instead we train on all sequences and only evaluate on long sequences?
  - \textit{CT} is again the best performer, but absolute performance is considerably worse.
...
- \textit{VSTAN\_EBR} performs well on early positions but performance deteriorates for longer test sequences.
...
- \textit{CT} and the three NN-based models do not show any clear trend across positions.
...
- Additionally, we would expect performance increases by non-session-aware models to be larger than those for session-aware models:
  - The long-term memory of session-aware models should already learn specific app predilections within an app category on a user level.
  - E.g., that user X mostly uses WhatsApp, whereas user Y mostly uses Telegram for messaging.
  - Since session-based models do not have this user-level memory, they should be more prone to getting the individual app wrong despite predicting the right category.
...
- Performance increases for non-neural methods are rather small when looking at the top-ranked recommendation but noticeable for large recommendation list lengths.
- Compared to the performance increase when dropping ON and OFF events, performance increases due to category-level prediction are approximately proportional to standard app-level performance.
- This implies that none of the non-neural algorithms get the category right many more times than the actual app, in particular when only considering the top-ranked recommendation.
- For the NN-based models, increases due to category-level evaluation are a bit more pronounced, especially for larger $k$.
- Conclusions:
  ...
  - We do not find any evidence of larger performance increases for models without user-level memory.
...
- Exemplarily, we illustrate two-dimensional app embeddings for apps from the categories \textit{Messaging} and \textit{Social Networks}.
...
- To quantify this, we also consider the share of apps whose category coincides with the category of its most similar app
- Similarity is computed in terms of cosine similarity based on original 128-dimensional embeddings.
- The share is $11.67\%$ $\rightarrow$ for less than one out of eight apps its nearest neighbor app is from the same category.
...
- For illustration, we again use two-dimensional embeddings (as above).
- To qualitatively assess whether these data-based clusters coincide with our pre-established app categories, we then look at potential accumulations of app categories within each one of the 15 clusters.
...
- We find that no meaningful app analogies from our embeddings:
...
- The average performance drop (across all algorithms) is $63.88\%$.
- Across all algorithms and evaluation metrics, the performance is now much worse than in the app-level setting.
- \textit{CT} is still the best performer, while the neural methods \textit{GRU4Rec} and \textit{HGRU4Rec} are the worst performers for all metrics.
...
- Difference in performance across positions for app- versus sequence-level data:
  - In the app-level setting, tuning and training was performed on predominantly short sequences
  - In the sequence-level setting, the large majority of sequences is long by construction.
...
- Potentially even bigger problem: insufficient data for proper learning of behavioral sequences, in particular in the sequence-level setting.
