...
![\tiny Table XXX: Excerpt of anonymized app-level data.](../tables/sample_excerpt.png){width=80% height=60%}
...
![\tiny Table XXX: Summary statistics of app-level and sequence-level data.](../tables/summary_statistics.png){width=100% height=75%}
...
- Preferable to \textit{last-item prediction}
...
1) Hit Rate (HR): $HR@k$ is simply the fraction of events for which the corresponding recommendation list of length $k$, $rl(k)_i$, includes the ground truth, $y_i$:
\begin{align*}
